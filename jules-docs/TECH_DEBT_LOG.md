...
| **TD-004** | P2 | Backend | `server/src/controllers/` | Optimized read-only database queries across `user.controller.js`, `challenge.controller.js`, `submission.controller.js`, and `dashboard.controller.js` using `.lean()` and replaced case-insensitive regex user lookups with `{ username: username.toLowerCase() }` for B-Tree index utilization. | Resolved | August 10, 2026 |

## Open Technical Debt Backlog
| ID | Priority | Layer | Component / Route | Description of Debt | Status |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **TD-005** | P3 | Services | `server/src/services/` | Consider introducing abstract repository pattern layer for complex queries spanning multiple models to further decouple Express handlers. | Open |
