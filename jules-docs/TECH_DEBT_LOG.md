...
| **TD-005** | P2 | Backend | `server/src/controllers/submission.controller.js` | Dynamic case-insensitive regex lookup `{ username: { $regex: ... } }` in `getSubmissionsByUsername` caused database table scans. Refactored to lowercased exact equality `{ username: username.toLowerCase() }` to utilize `{ username: 1 }` B-Tree index. | Resolved | August 12, 2026 |
