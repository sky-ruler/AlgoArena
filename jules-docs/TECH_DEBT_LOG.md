...
| **TD-004** | P2 | Database / Backend | `server/src/controllers/` | Case-insensitive username lookups used dynamic regex queries (`{ username: { $regex: ... } }`) causing full collection scans and bypassing the index. Standardized all username lookups to lowercased exact equality `{ username: username.toLowerCase() }`, enabling B-Tree index utilization. | Resolved | August 10, 2026 |

## Open Technical Debt Backlog
| ID | Priority | Layer | Component / Route | Description of Debt | Status |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **TD-005** | P2 | Backend | `server/src/controllers/` | Inconsistent patterns for direct manual collection queries vs. abstract service wrapper usage. Standardizing on a repository or dedicated service layer would improve maintainability. | Open |
