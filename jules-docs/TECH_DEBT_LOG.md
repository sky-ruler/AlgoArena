...
| **TD-005** | P2 | Database / Backend | `server/src/controllers/` | Username queries in `getUserProfile` and `getSubmissionsByUsername` used unindexed `$regex` searches. Replaced with indexed exact lowercase queries `{ username: username.toLowerCase() }` and deleted leftover artifact `Clan.fixed.js`. | Resolved | August 10, 2026 |
