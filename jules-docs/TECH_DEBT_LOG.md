...
| **TD-005** | P2 | Database / Backend | `server/src/controllers/` | Un-indexed `$regex` username lookups in auth, submission, badge, and dashboard controllers bypassed B-tree index. Converted to lowercased exact queries (`username.toLowerCase()`) utilizing native `{ username: 1 }` index and deleted residual file `Clan.fixed.js`. | Resolved | August 10, 2026 |
