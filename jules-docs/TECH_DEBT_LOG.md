...
| **TD-005** | P2 | Backend | `server/src/controllers/` | Read-only endpoints in `challenge.controller.js` and `submission.controller.js` previously generated full Mongoose document instances for read operations. Optimized by chaining `.lean()` to reduce memory allocation and speed up API responses. | Resolved | August 10, 2026 |
