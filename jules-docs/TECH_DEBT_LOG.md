...
| **TD-005** | P2 | Backend / Database | `server/src/controllers/badge.controller.js` | `getBadgesForUsername` used dynamic case-insensitive `$regex` query, preventing schema B-Tree index utilization. Refactored to query `{ username: username.toLowerCase() }` for fast B-Tree lookup. | Resolved | August 10, 2026 |
