...
| **TD-005** | P1 | Database | `server/src/models/` | AuditLog, Resource, and QuestionSet Mongoose models lacked field indexes on high-frequency query fields (`targetUserId`, `performedBy`, `timestamp`, `folder`, `uploadedBy`, `weekNumber`, `deadline`, `status`). Added explicit indexes to optimize query execution and prevent collection scans. | Resolved | August 10, 2026 |
| **TD-006** | P2 | Backend | `resource.controller.js` & `questionSet.controller.js` | Read-only endpoints (`getResources`, `getQuestionSets`, `getQuestionSetById`) hydrated full Mongoose documents unnecessarily. Appended `.lean()` to return plain JS objects, reducing memory overhead and response latency. | Resolved | August 10, 2026 |
