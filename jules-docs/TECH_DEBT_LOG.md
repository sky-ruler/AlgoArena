...
| **TD-003** | P2 | Cache | `server/src/services/clanScope.service.js` | Clan chief lookups use a simple local in-memory cache variable (`chiefClanCache`). This works beautifully in single-process mode, but will lead to cache inconsistency in a multi-instance stateless container/cluster. Need to migrate to Redis. | Open |
