...
| **TD-005** | P2 | Backend | `server/src/controllers/badge.controller.js` & `resource.controller.js` | Un-optimized read queries (`getResources` missing `.lean()`, `getBadgesForUsername` using dynamic regex instead of B-tree index). Resolved by appending `.lean()` and querying with lowercased username. | Resolved | August 10, 2026 |
