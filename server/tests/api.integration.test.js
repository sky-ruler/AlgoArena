...
test('clan chief lookup implements negative caching to prevent database stampedes for non-chief users', async () => {
  const { chiefClanCache } = require('../src/services/clanScope.service');
  ...
  try {
    ...
    const cached = await chiefClanCache.get(regularUser.id);
...
test('Badge awarding and revoking enforces strict RBAC (only chief/admin, no self-awarding, no cross-clan)', async () => {
  ...
  assert.ok(updatedMember.awardedBadgeIds.some((id) => id.toString() === badge._id.toString()));
