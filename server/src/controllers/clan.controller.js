...
const {
  ...
  toIdString,
} = require('../services/clanScope.service');
const { computeSetAnalytics, deriveDisplayStatus } = require('../services/setAnalytics.service');
...
const getClans = async (req, res, next) => {
  try {
    // Optimization: Restrict populated fields on members and chief to avoid retrieving
    // heavy payloads (such as base64 profile pictures) for the entire list of clans.
    // Also skip unused populates (requests, createdBy, archivedBy, restoredBy).
...
const getClanLeaderboard = async (req, res, next) => {
  try {
    ...
    // Single aggregation across all members — group by userId (optimized to avoid expensive and redundant challenges lookup)
